...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if torch.nonzero(keep_bool).shape==torch.Size([]):
        return Variable(torch.zeros((1,6)).cuda())
...
# TODO: can be replaced by the deep learning v2 network
...
class MaskRCNN(nn.Module):
    ...
    def set_trainable(self, layer_regex, model=None, indent=0, verbose=1):
        ...
        for param in self.named_parameters():
            layer_name = param[0]
            trainable = bool(utils.fullmatch(layer_regex, layer_name))
            if not trainable:
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers, file_name=None):
        """Train the model.
        ...
        train_generator = torch.utils.data.DataLoader(train_set, batch_size=1, shuffle=True, num_workers=0)
        val_set = Dataset(val_dataset, self.config, augment=True)
        val_generator = torch.utils.data.DataLoader(val_set, batch_size=1, shuffle=True, num_workers=0)
        ...
        for epoch in range(self.epoch+1, epochs+1):
            ...
            if file_name == None:
                ...
                torch.save(self.state_dict(), './checkpoints/'+file_name+'.pth')
        self.epoch = epochs
