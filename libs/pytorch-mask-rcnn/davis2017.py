...
with open(args.dataset+'/trainval/ImageSets/2017/train.txt') as f:
    fr = f.readlines()
    file_namelist1 = [x.strip() for x in fr]
    file_namelist.extend(file_namelist1)
with open(args.dataset+'/trainval/ImageSets/2017/val.txt') as f:
    fr = f.readlines()
    file_namelist2 = [x.strip() for x in fr]
    file_namelist.extend(file_namelist2)
#print(file_namelist)
for file in file_namelist:
    AugmentImgPath = os.path.join(args.dataset, 'trainval', 'JPEGImages', '480p', file)
    AugmentAnnoPath = os.path.join(args.dataset, 'trainval', 'Annotations', '480p', file)
    obj_num = get_obj_num(AugmentAnnoPath+'/00000.png') - 1
    ...
    test_files.extend([x[:-4] for x in file_names[:int(args.limit)]])
    train_files.extend([x[:-4] for x in file_names[int(args.limit):]])
    train_name.extend(file for x in range(len(file_names) - int(args.limit)))
    test_name.extend(file for x in range(int(args.limit)))
...
class DavisDataset(utils.Dataset):
    def load_davis(self, subset):
        ...
        for i in range(len(image_list)):
            AugmentImgPath = os.path.join(args.dataset, 'trainval', 'JPEGImages', '480p', name_list[i])
            AugmentAnnoPath = os.path.join(args.dataset, 'trainval', 'Annotations', '480p', name_list[i])
            if get_obj_num(os.path.join(AugmentAnnoPath, image_list[i]+'.png')) == 1:
                miss_mask += 1
                continue
            self.add_image(
                "davis", image_id=i,
                path=os.path.join(AugmentImgPath, image_list[i]+'.jpg'),
                annotations=os.path.join(AugmentAnnoPath, image_list[i]+'.png'))
