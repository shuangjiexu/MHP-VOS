...
# Custom includes
from dataloaders import pascal, sbd, combine_dbs, cocodataset
...
def adjust_lr(optimizer, itr, max_itr):
    now_lr = 0.01 * (1 - itr/(max_itr+1)) ** 0.9
    for group in optimizer.param_groups:
        group['lr'] = now_lr
    if max_itr / (itr+1) == 0:
        print('(poly lr policy) learning rate: ', now_lr)
    return now_lr
...
if backbone == 'xception':
    ...
    net = deeplab_resnet.DeepLabv3_plus(nInputChannels=3, n_classes=91, os=16, pretrained=True)
else:
...
if resume_epoch != nEpochs:
    ...
    voc_train = cocodataset.COCODataset(dataset_name='coco2014', rootdir='../cocodata', period='train')
    ...
    # db_train = db.DAVIS2016(train=True, transform=composed_transforms)
    ...
    itr = 0 * len(trainloader)
    max_itr = 30*len(trainloader)
    print("Training Network")

    # checkpoint_epoch = 29
    # print('load checkpoint '+str(checkpoint_epoch))
    # net.load_state_dict(torch.load('./run//run_10/models/deeplabv3plus-resnet-voc_epoch-29.pth'))
    ...
    for epoch in range(0, 30):
        ...
        for ii, sample_batched in enumerate(trainloader):
            ...
            inputs, labels = sample_batched['image'], sample_batched['segmentation']
            inputs = inputs.squeeze(0)
            labels = labels.squeeze(0)
            # print(inputs.shape)
            # Forward-Backward of the mini-batch
            inputs, labels = Variable(inputs, requires_grad=True), Variable(labels.long())
        ...
        if (epoch % snapshot) == snapshot - 1:
            ...
            print("Save model at {}\n".format(os.path.join(save_dir, modelName + '_epoch-' + str(epoch) + '.pth')))
