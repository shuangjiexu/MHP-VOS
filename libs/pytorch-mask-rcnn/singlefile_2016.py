...
#with open('../DAVIS2017/test_dev/ImageSets/2017/test-dev.txt') as f:
#    fr = f.readlines()
#    file_namelist = [x.strip() for x in fr]
file_namelist = ['blackswan','bmx-trees','breakdance','camel','car-roundabout',
...
def get_file(file):
    ...
    for num in range(200):
        AugmentImgPath = os.path.join(args.dataset, file)
        AugmentAnnoPath = os.path.join(args.dataset, file)
        obj_num = 1
...
class DavisDataset(utils.Dataset):
    def load_davis(self, subset, file):
        ...
        for i in range(len(image_list)):
            AugmentImgPath = os.path.join(args.dataset, name_list[i])
            AugmentAnnoPath = os.path.join(args.dataset, name_list[i])
