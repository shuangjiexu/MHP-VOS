...
def get_file(file):
    ...
    for num in range(200):
        AugmentImgPath = os.path.join(args.dataset, 'test_lucid_dataset', file)
        AugmentAnnoPath = os.path.join(args.dataset, 'test_lucid_dataset', file)
        obj_num = get_obj_num('../data/DAVIS/test_dev/Annotations/480p/'+file+'/00000.png') - 1
...
class DavisDataset(utils.Dataset):
    def load_davis(self, subset, file):
        ...
        for i in range(len(image_list)):
            AugmentImgPath = os.path.join(args.dataset, 'test_lucid_dataset', name_list[i])
            AugmentAnnoPath = os.path.join(args.dataset, 'test_lucid_dataset', name_list[i])
