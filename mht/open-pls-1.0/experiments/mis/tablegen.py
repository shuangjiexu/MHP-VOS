...
#        sys.exit(1)
    ...
    #avg mis size
    reduced_array.append("{0:.2f}".format(sum([float(x) for x in hash_list["mis"]])/len(hash_list["mis"])))

    #max mis size
    max_mis_string = str(max([int(x) for x in hash_list["mis"]]))
    reduced_array.append(max_mis_string)

    #number achieving max mis size
    reduced_array.append(str(hash_list["mis"].count(max_mis_string)))
