...
from dataloaders import lucid_dataset_2016 as db
...
# seq_name_list = ['aerobatics', 'car-race', 'carousel', 'cats-car', 'chamaleon', 'deer', 'giant-slalom',
#                  'girl-dog', 'golf', 'guitar-violin', 'gym', 'helicopter', 'horsejump-stick', 'hoverboard',
#                  'lock', 'man-bike', 'monkeys-trees', 'mtb-race', 'orchid', 'people-sunset', 'planes-crossing',
#                  'rollercoaster', 'salsa', 'seasnake', 'skate-jump', 'slackline', 'subway', 'tandem',
#                  'tennis-vest', 'tractor']
# seq_name_list = ['blackswan', 'bmx-trees', 
...
if resume_epoch != nEpochs:
    ...
    for seq_name in seq_name_list:
        obj_num = get_obj_num('../data/DAVIS/trainval/Annotations/480p/'+seq_name+'/00000.png')
        ...
        for obj_id in range(2):
            ...
            net.load_state_dict(torch.load('./checkpoints/mask_final.pth'))
            ...
            for epoch in range(resume_epoch, nEpochs):
                ...
                #         aveGrad = 0
                if epoch%10==9:
                    net.load_state_dict(torch.load('/media/ubuntu/ouyang11/VOS/model/'+seq_name+'_'+str(obj_id)+'_'+str(epoch)+'.pth'))
                    print("load model at {}\n".format('/media/ubuntu/ouyang11/VOS/model/'+seq_name+'_'+str(obj_id)+'_'+str(epoch)+'.pth'))  
                ...
                # if useTest and epoch % nTestInterval == (nTestInterval - 1):
                    ...
                    img_path = np.sort(os.listdir(os.path.join('../data/DAVIS/JPEGImages/480p', seq_name)))
                    img_list = list(map(lambda x: os.path.join('../data/DAVIS/JPEGImages/480p', seq_name, x), img_path))[1:]        

                    mask_path = os.path.join('../data/DAVIS/Annotations/480p', seq_name)
                    mask = os.path.join(mask_path, '00000.png')     

                    label_path = np.sort(os.listdir(os.path.join('../data/DAVIS/Annotations/480p', seq_name)))
                    label_list = os.path.join('../data/DAVIS/Annotations/480p', seq_name, label_path[0])
                    # label_list = list(map(lambda x: os.path.join('../DAVIS2017/test_dev/Annotations/480p', seq_name, x), label_path))[1:]          

                    f_mask_ = utils.imread_indexed_255(mask)
                    # _, palette = utils.imread_indexed(mask)
                    # print(palette.shape)
                    f_mask = f_mask_.copy()
                    mask = np.array(f_mask, dtype=np.float32)/255
                    h, w = mask.shape     
                    ...
                    for ii in range(len(img_list)):
                        ...
                        # plt.pause(0.01)     
                            ...
                            # if not os.path.exists('/media/ubuntu/ouyang11/VOS/mask_result'):
                            #     os.mkdir('/media/ubuntu/ouyang11/VOS/mask_result')
                            # if not os.path.exists('/media/ubuntu/ouyang11/VOS/mask_result/'+seq_name):
                            #     os.mkdir('/media/ubuntu/ouyang11/VOS/mask_result/'+seq_name)
                            # if not os.path.exists('/media/ubuntu/ouyang11/VOS/mask_result/'+seq_name+'/'+str(obj_id)):
                            #     os.mkdir('/media/ubuntu/ouyang11/VOS/mask_result/'+seq_name+'/'+str(obj_id))
                            # if not os.path.exists('/media/ubuntu/ouyang11/VOS/mask_result/'+seq_name+'/'+str(obj_id)+'/'+str(epoch)):
                            #     os.mkdir('/media/ubuntu/ouyang11/VOS/mask_result/'+seq_name+'/'+str(obj_id)+'/'+str(epoch))
                            # save_path = os.path.join('/media/ubuntu/ouyang11/VOS/mask_result',seq_name,str(obj_id),str(epoch),str(ii+1).zfill(5)+'.png')
                            # # print(mask)
