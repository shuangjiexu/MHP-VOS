...
class ASPP(nn.Module):
	...
	def __init__(self, dim_in, dim_out, rate=1):
		...
		self.branch2 = nn.Sequential(
				nn.Conv2d(dim_in, dim_out, 3, 1, padding=6*rate, dilation=6*rate),
				#nn.BatchNorm2d(dim_out),
				SynchronizedBatchNorm2d(dim_out),
				nn.ReLU(inplace=True),	
		)
		self.branch3 = nn.Sequential(
				nn.Conv2d(dim_in, dim_out, 3, 1, padding=12*rate, dilation=12*rate),
				#nn.BatchNorm2d(dim_out),
				SynchronizedBatchNorm2d(dim_out),
				nn.ReLU(inplace=True),	
		)
		self.branch4 = nn.Sequential(
				nn.Conv2d(dim_in, dim_out, 3, 1, padding=18*rate, dilation=18*rate),
				#nn.BatchNorm2d(dim_out),
				SynchronizedBatchNorm2d(dim_out),
				nn.ReLU(inplace=True),	
		)
