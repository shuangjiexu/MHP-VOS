...
#        sys.exit(1)
    ...
    #avg mwc size
    reduced_array.append("{0:.2f}".format(sum([float(x) for x in hash_list["mwc"]])/len(hash_list["mwc"])))

    #max mwc size
    max_mwc_string = str(max([int(x) for x in hash_list["mwc"]]))
    reduced_array.append(max_mwc_string)

    #number achieving max mwc size
    reduced_array.append(str(hash_list["mwc"].count(max_mwc_string)))
